...
function WoWTranslate_API.ConfigureAzure(apiKey, region, endpoint)
    if not dllAvailable then
        return false, "DLL not available"
    end

    local success, result = pcall(function()
        return UnitXP("WoWTranslate", "configure_azure",
            apiKey or "",
            region or "",
            endpoint or "https://api.cognitive.microsofttranslator.com")
    end)
...
function WoWTranslate_API.HasSavedProviderConfig(provider)
    ...
    elseif provider == "azure" then
        return WoWTranslateDB.azureApiKey and WoWTranslateDB.azureApiKey ~= ""
...
function WoWTranslate_API.ConfigureFromSaved(force)
    ...
    elseif provider == "azure" then
        return WoWTranslate_API.ConfigureAzure(
            WoWTranslateDB.azureApiKey or "",
            WoWTranslateDB.azureRegion or "",
            WoWTranslateDB.azureEndpoint or "https://api.cognitive.microsofttranslator.com")
    elseif provider == "openai" then
