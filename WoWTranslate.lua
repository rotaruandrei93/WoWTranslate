...
local defaults = {
    ...
    -- the DLL pre-configured with the Ocp-Apim-Subscription-Key, so it works
    -- with zero in-game setup. This Lua-side "azure" entry stays unconfigured
...
local PROVIDER_LABELS = {
    ...
    azure = "Azure Translator (Microsoft)",
...
local function NormalizeProvider(provider)
    ...
    if provider == "microsoft" or provider == "azure_translator" then
        provider = "azure"
    end
    if provider == "google" or provider == "google_free" or provider == "azure" or provider == "openai" or provider == "custom" then
        return provider
    end
    return nil
end
...
SlashCmdList["WOWTRANSLATE"] = function(msg)
    ...
    elseif cmd == "provider" then
        if arg and arg ~= "" then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("  Usage: /wt provider google_free|google|azure|openai|custom")
        end

    elseif cmd == "azurekey" and arg then
        WoWTranslateDB.provider = "azure"
        WoWTranslateDB.azureApiKey = arg
        local success, err = WoWTranslate_API.ConfigureAzure(arg, WoWTranslateDB.azureRegion, WoWTranslateDB.azureEndpoint)
        if success then
            DEFAULT_CHAT_FRAME:AddMessage("|cFF00FF00[WoWTranslate] Azure Translator API key set|r")
        else
            DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[WoWTranslate] Failed to set Azure key: " .. (err or "unknown") .. "|r")
        end

    elseif cmd == "azureregion" then
        WoWTranslateDB.azureRegion = arg or ""
        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] Azure region set to " .. (arg == "" and "(global, no region)" or arg))
        if WoWTranslateDB.provider == "azure" then
    ...
    elseif cmd == "hide" then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("  /wt provider google_free|google|azure|openai|custom")
        DEFAULT_CHAT_FRAME:AddMessage("  /wt googlekey <key>  (/wt key is deprecated)")
        DEFAULT_CHAT_FRAME:AddMessage("  /wt azurekey <key> | /wt azureregion <region>")
